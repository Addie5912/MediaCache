...
func NewVideoService(r remote.Remote, s storage.Storage, a AlarmService) VideoService {
...
func (v *VideoServiceImpl) GetVideo(videoPath string) (io.ReadCloser, *storage.FileInfo, error) {
	...
	if conf.IsCacheAvailable() && v.storage.Exist(videoPath) {
		...
		if err == nil {
			logger.Infof("[VideoService] cache hit for video: %s", videoPath)
	...
	if err != nil {
		...
		v.alarm.SendAlarm(AlarmId300020, fmt.Sprintf("get video from MUEN failed: %v", err))
		return nil, nil, err
	}
